...
# 来自前端结构化表单的消息模式（直接匹配，不再二次解析）
_FORM_WISH_PATTERN = re.compile(
    r"我设置了「(.+?)」心愿，目标\s*([\d.]+)\s*元，已攒\s*([\d.]+)\s*元，计划\s*(\d+)\s*个月完成，每月需攒\s*([\d.]+)\s*元"
)
...
class ReversePlannerNode:
    def __call__(self, state: FinBuddyState) -> dict:
        ...
        # ── 优先处理来自前端结构化表单的消息 ──
        form_match = _FORM_WISH_PATTERN.search(text)
        if form_match:
            name = form_match.group(1)
            target = float(form_match.group(2))
            saved = float(form_match.group(3))
            months = int(form_match.group(4))
            monthly_saving = float(form_match.group(5))
            remaining = max(0.0, target - saved)

            # 直接用前端已经更新好的 wishlists 状态，只生成 AI 鼓励反馈
            plan_desc = (
                f"用户通过心愿表单设置了「{name}」，目标{target:.2f}元，"
                f"已攒{saved:.2f}元，还差{remaining:.2f}元，"
                f"计划{months}个月完成，每月建议储蓄{monthly_saving:.2f}元。"
                "请用同龄学长口吻给予温暖鼓励，肯定用户的规划行为，并给1条实用的攒钱小建议，不超过80字。"
            )
            fallback = (
                f"「{name}」心愿已经规划好了！目标 {target:.2f} 元，每月存 {monthly_saving:.2f} 元，"
                f"{months} 个月后就能实现 🎯 把心愿资金放进货币基金和日常花销隔开，会更有仪式感哦！"
            )
            reply = generate_finbuddy_reply(
                task=plan_desc,
                user_text=text,
                state={**state, "intent": "心愿规划", "wishlists": wishlists, "active_wishlist_index": active_idx},
                fallback=fallback,
            )
            return {
                "wishlists": wishlists,
